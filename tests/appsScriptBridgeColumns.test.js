...
test('booking CRM headers append after occupied spreadsheet columns', () => {
    ...
    assert.deepEqual(sheet.writes.map(write => write.column), [26, 27, 28, 29, 30]);
    assert.deepEqual(sheet.headers.slice(25, 30), [
        'contactStatus',
        'draftStatus',
        'lastContactedDate',
        'nextFollowUpDate',
...
test('existing booking CRM headers are reused instead of duplicated', () => {
    ...
    assert.deepEqual(sheet.writes.map(write => write.column), [29, 30, 31]);
...
test('preferred map columns append safely when their slots are occupied', () => {
    ...
    assert.deepEqual(sheet.writes.map(write => write.column), [21, 22, 23, 24, 25, 26, 27, 28]);

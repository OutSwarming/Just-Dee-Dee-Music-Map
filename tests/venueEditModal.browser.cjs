...
await card(0).locator('[aria-label="Notes for email 2"]').click();await card(0).locator('#venue-contact-0-emails-1-note').fill('Gig materials; "after 5"\nSecond line');await card(0).locator('[data-contact-group="emails"] [data-note-done]').nth(1).click();
...
let saved=await page.evaluate(()=>window.JDDMContacts.read(window.saved));assert.equal(saved.contacts.length,2);assert.equal(saved.contacts[0].emails[1].note,'Gig materials; "after 5"\nSecond line');assert.equal(saved.contacts[1].others[0].type,'Messenger');assert.equal(await page.evaluate(()=>Object.hasOwn(window.saved,'Contact Details')),false);
...
await page.evaluate(()=>window.BARK.openVenueEditor({id:'test',name:'JDDM Contact Editor Test'}));await expect(card(1).getByRole('textbox',{name:'Phone 1',exact:true})).toHaveValue('330-555-0199');await expect(card(0).getByRole('textbox',{name:'Phone 1',exact:true})).toHaveValue('+1 (330) 555-0100 ext 2');
...
await card(1).locator('[data-person-remove]').click();await page.locator('#venue-edit-save').click();await expect(page.locator('#venue-edit-status')).toContainText('Saved');assert.equal((await page.evaluate(()=>window.JDDMContacts.read(window.saved))).contacts.length,1);

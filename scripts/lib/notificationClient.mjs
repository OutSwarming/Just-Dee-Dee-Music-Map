...
export async function dailyDigest(){const c=await config();const r=await fetch('https://us-central1-just-dee-dee-music-map.cloudfunctions.net/jddmNotificationDigest',{headers:{'x-jddm-key':c.digestKey},signal:AbortSignal.timeout(60000)});const d=await r.json();if(!r.ok||!d.ok)throw Error('Daily follow-up list could not be loaded');return d;}
export async function postNotification(kind,body,{key,fetchImpl=fetch}={}){
 const c=await config(),channel=c.channels[kind];if(!channel)throw Error('Unconfigured Discord notification kind: '+kind);
 const id=createHash('sha256').update(key||kind+'|'+body).digest('hex');const dir=path.join(SUPPORT,'discord-notification-receipts');await mkdir(dir,{recursive:true});const file=path.join(dir,id+'.json');let receipt={next:0,kind,body,key:key||kind+'|'+body};try{receipt=JSON.parse(await readFile(file,'utf8'));}catch{}if(receipt.sent)return {duplicate:true,channel};
...
export async function retryNotifications(){
 ...
 for(const file of files.filter(n=>n.endsWith('.json'))){const r=JSON.parse(await readFile(path.join(dir,file),'utf8'));if(!r.sent&&r.kind&&r.body)await postNotification(r.kind,r.body,{key:r.key});}
}

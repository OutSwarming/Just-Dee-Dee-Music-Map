...
export async function gatherSources({now=new Date(),settings,discord,loadDaily=()=>dailyDigest({summary:true}),previousGigKeys=null,fetchImpl=fetch}){
 ...
 const priority=s=>['M','TODAY'].includes(s.id)?0:s.label.startsWith('Email:')?1:s.label.includes('follow-up list')?2:s.label.startsWith('Calendar:')?3:4;sources.sort((a,b)=>priority(a)-priority(b));

...
export function currentVenueReviews(rows,conversations,today){
 return rows.flatMap(row=>{
  ...
  if(!date&&!['rejected','resolved','followup'].includes(latest?.status))return [];
  return [{name:row['Place Name'],city:row.City||'',officialFollowUp:date||'',inDailyWindow:!!date&&date>=today&&date<=plusDays(today,2),followUpDue:!!date&&date===today&&!/^(Open Microphone|Told No)/i.test(row.Status||''),savedStatus:row.Status||'',savedContactInfo:{name:row['Contact Name']||'',hasEmail:!!row['Email/Contact'],hasPhone:!!row['Phone Number']},lastContacted:row['Last Contacted']||'',notes:String(row.Notes||'').slice(0,700),...(latest?{latestConversation:{status:latest.status,subject:String(latest.subject||'').slice(0,250),preview:String(latest.preview||'').slice(0,500),lastMessageAt:latest.lastMessageAt||latest.lastStatusMessageAt||0}}:{})}];
 });
}
export async function gatherSources({now=new Date(),settings,discord,loadDaily=()=>dailyDigest({summary:true}),previousGigKeys=null,fetchImpl=fetch}){
 const today=dateKey(now),sources=[],warnings=[];
 const add=(label,text,url)=>sources.push({id:'S'+(sources.length+1),label,text:String(text).slice(0,5000),url});
 // Rebuild the list and all metrics from ONE current sheet snapshot. Never use
 // yesterday's/frozen morning body as an authority for the model.
 const digest=await loadDaily();
 if(!Array.isArray(digest.conversations))throw Error('Current email status is unavailable');
 const response=await fetchImpl('https://us-central1-just-dee-dee-music-map.cloudfunctions.net/jddmSpreadsheetBridge?action=csv',{signal:AbortSignal.timeout(30000)});
 if(!response.ok)throw Error('Current spreadsheet unavailable; refusing stale follow-ups');
 const data=sheetBridge.parseCsv(await response.text()),headers=data.shift();
 if(!headers?.includes('Future Gigs')||!headers.includes('Next Follow Up'))throw Error('Invalid current spreadsheet');
 const rows=data.map(row=>Object.fromEntries(headers.map((h,i)=>[h,row[i]||''])));
 const byId=new Map(rows.map(r=>[r['Place ID'],r]));
 const conversations=digest.conversations.map(c=>{
  if(!c.venueId)return {...c,followUpDate:venueDate(c.followUpDate)};
  const row=byId.get(c.venueId);
  return {...c,venueName:row?.['Place Name']||c.venueName,followUpDate:venueDate(row?.['Next Follow Up']),followUpDateSource:row?'current spreadsheet':'linked venue unavailable'};
 });
 const fresh=digestDates.buildDigest({rows,emails:conversations,worklist:digest.followUpContext?.worklist||[],booking:digest.followUpContext?.booking||[],today});
 add('Today’s follow-up list',fresh.body,`https://discord.com/channels/${GUILD}/${settings.channels['daily-follow-ups']}`);
 const metrics=calculateMetrics({rows,conversations,today,previousGigKeys});
 // Accurate backlog counts stay available to trackers, but are not AI inputs or
 // morning metrics. Hiding a backlog never changes the saved spreadsheet dates.
 const {overduePlaces,overdueEmails,...morningMetrics}=metrics;
 const todayPlan=buildTodayPlan({rows,calendar:digest.calendar,today,metrics});
 sources.unshift({id:'TODAY',label:'Today’s verified gigs and locations',text:JSON.stringify(todayPlan),url:'https://outswarming.github.io/Just-Dee-Dee-Music-Map/'});
 sources.unshift({id:'M',label:'Exact metrics calculated locally from current saved records',text:JSON.stringify(morningMetrics),url:'https://outswarming.github.io/Just-Dee-Dee-Music-Map/'});
 if(metrics.invalidGigRows)warnings.push(`${metrics.invalidGigRows} venue rows have unclear future gig dates and need review.`);
 const currentReviews=currentVenueReviews(rows,conversations,today);
 for(let i=0;i<currentReviews.length;i+=2)add('Current saved venue review status',JSON.stringify(currentReviews.slice(i,i+2)),'https://outswarming.github.io/Just-Dee-Dee-Music-Map/');
 if(digest.calendar){
  const seen=new Set(),end=plusDays(today,7);
  const events=digest.calendar.calendars.flatMap(c=>c.events).filter(e=>{const d=dateKey(new Date(e.start)),key=e.title+'|'+e.start;if(d<today||d>end||seen.has(key))return false;seen.add(key);return true;}).sort((a,b)=>a.start.localeCompare(b.start));
  add('Calendar: today and next seven days',JSON.stringify(events.map(e=>({title:e.title,start:new Date(e.start).toLocaleString('en-US',{timeZone:'America/New_York'}),end:new Date(e.end).toLocaleString('en-US',{timeZone:'America/New_York'}),allDay:e.allDay,location:e.location}))),`https://discord.com/channels/${GUILD}/${settings.channels['calendar-changes']||'1546426968784769074'}`);
 }else warnings.push('The calendar snapshot is unavailable or stale; upcoming gigs could not be fully checked.');
 for(const [name,id] of Object.entries(settings.channels)){
  ...
  try{
   const messages=recentMessages(await discord(`/channels/${id}/messages?limit=100`),now);
   if(messages.length>8)warnings.push(`${name}: showing the latest 8 updates from the last 24 hours.`);
   for(const m of messages.slice(0,8)){
    const text=(m.content||'')+' '+(m.embeds||[]).map(e=>[e.title,e.description].filter(Boolean).join(': ')).join('\n');
    // Historical reminder posts are not current tasks, even if posted today.
    if(/over[ -]?due|past[ -]?due|Daily follow-ups|places you need to contact today/i.test(text))continue;
    add(name,text,`https://discord.com/channels/${GUILD}/${id}/${m.id}`);
   }
  }catch{warnings.push(`${name} could not be checked.`);}
 }
 const order={deedee:0,followup:1,venue:2};
 const waiting=conversations.filter(c=>['deedee','venue'].includes(c.status)||(c.status==='followup'&&c.followUpDate>=today&&c.followUpDate<=plusDays(today,2))).sort((a,b)=>order[a.status]-order[b.status]||Number(b.lastMessageAt)-Number(a.lastMessageAt));
 for(const c of waiting.slice(0,50))add('Email: '+c.subject,JSON.stringify({...c,statusMeaning:c.status==='venue'?'Waiting on VENUE: Dee Dee has already sent her response; do not say Dee Dee owes a reply.':c.status==='deedee'?'Waiting on DEE DEE: review and respond as appropriate.':'Follow-up is scheduled in the current three-day window; use the current followUpDate.',lastMessageEastern:c.lastMessageAt?new Date(Number(c.lastMessageAt)).toLocaleString('en-US',{timeZone:'America/New_York'}):'Unknown'}),`https://discord.com/channels/${GUILD}/${c.discordThreadId}`);
 ...
 const priority=s=>(['M','TODAY'].includes(s.id)||s.label.includes('follow-up list'))?0:s.label==='Current saved venue review status'?1:s.label.startsWith('Email:')?2:s.label.startsWith('Calendar:')?3:4;
 sources.sort((a,b)=>priority(a)-priority(b));
 let size=0;const selected=[];
 for(const s of sources){if(size+s.text.length>48000){warnings.push('Some additional detail was omitted to keep the recap concise.');break;}selected.push(s);size+=s.text.length;}
 return {today,generatedAt:now.toISOString(),metrics:morningMetrics,todayPlan,sources:selected,warnings:[...new Set(warnings)]};
}
...
export function usefulBullets(summary,input){
 const sources=new Map(input.sources.map(s=>[s.id,s]));return summary.bullets.filter(b=>!/(?:over[ -]?due|past[ -]?due|missed follow[ -]?ups?|late follow[ -]?ups?)/i.test(b.text)&&b.sources.some(id=>!isSetupOrTest(sources.get(id)?.text)));
}
...
export function renderTextSummary(summary,input,{preview=false}={}){
 ...
 if(m)lines.push('',`Waiting: Dee Dee ${m.waitingOnDeeDee} conversations / venues ${m.waitingOnVenue}.`,`Remaining this week: ${m.followUpPlacesThisWeek} place follow-ups + ${m.followUpEmailsThisWeek} email follow-ups.`,`Today: ${m.followUpPlacesToday} place follow-ups + ${m.followUpEmailsToday} email follow-ups. Next 2 days: ${m.followUpPlacesNextTwoDays} places + ${m.followUpEmailsNextTwoDays} emails.`,`Booked ahead: ${m.totalBookedGigs} gigs / ${m.bookedLocations} locations${m.lastBookedDate?' through '+m.lastBookedDate:''}.`,`New gig dates: ${m.newRecordedGigs===null?'tracking starts today':m.newRecordedGigs}.`);

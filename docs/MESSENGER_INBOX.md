...
## Live Facebook round-trip verification — September 7, 2026

A controlled test used Carter Swarm's own Facebook account and the JDDM business Page, without messaging a venue. Carter's incoming message was retrieved by the official API and posted silently in Discord conversation `1546636594432315423` (`t_1791868155309568`), with blue Waiting on DEE DEE. A RESPONSE sent via the official Page Send API returned HTTP 200 and a message ID. The reply was visibly received in Carter's Facebook chat. The deployed scheduled poll copied the outgoing message into the same Discord post and changed its state to yellow Waiting on venue; both messages and the starter remained silent. The test conversation was subsequently marked resolved and `testConversation: true`.

This verifies sending to the app administrator inside a fresh messaging window, not unrestricted outreach to all customers. The Discord reply control still opens Meta; a direct-send Discord composer has not been implemented.

The Instagram Page endpoint, inbox/other folders, and Business Suite still return no conversations. Carter has partial Instagram Messages access. The direct Instagram browser session was signed in as `outswarming`; the `justdeedeemusic` login is required to inspect original history and the Connected tools / Allow access to messages setting. No Instagram send/receive round trip is verified yet. Meta's account help: https://www.facebook.com/help/instagram/791161338412168 .

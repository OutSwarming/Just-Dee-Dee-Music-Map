...
Early-preparation validation: 12 AI-summary tests and 16 notification tests passed, including changes at 8:05, partial delivery retries, local-only model inventory, DST and midnight boundaries. Two local model previews were reviewed and the prompt tightened to avoid unsupported booking/status claims. Live checks verified all 11 routine destinations (including the email forum) were quiet, while Daily Follow-ups and the AI recap each produced an everyone alert only on the first part. Temporary routing test posts were removed. The reviewed AI recap remains in its channel; separate recap test texts to Dee Dee and Carter both showed Delivered. The installed LaunchAgent contains 6:00, 7:45, and 8:05 entries plus five-minute retries.

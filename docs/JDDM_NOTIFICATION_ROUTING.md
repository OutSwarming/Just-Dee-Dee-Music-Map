...
### Recovery and runtime selection

The Homebrew Node executable was a separate ad-hoc-signed runtime; its Messages permission stayed off even after the user attempted authentication. The Mac also had an intact Node.js Foundation-signed Node 24.15.0 installation at the user’s NVM path, already used by existing background software. The two morning workers now run with that signed runtime. Its path is saved privately as `messagesNodePath` in notification-routing.json, and both installers preserve that choice rather than silently switching runtime identity to whichever node launches the installer. An unavailable selected runtime causes installation to fail instead of falling back to an unapproved executable. No privacy database was changed and no system protections were disabled.

Actual launchd recovery on September 7: follow-up receipts recorded Carter at 10:30:09 AM Eastern and Dee Dee at 10:30:12; the AI recap completed both remaining Messages destinations by 10:30:49. Dee Dee’s follow-up and recap both visibly showed Delivered in Messages. Discord delivery was already successful and was not repeated. These checks used the real background jobs, not a manually pasted test.

At 10:35:05 Eastern, launchd automatically spawned both jobs from their calendar events. Both exited successfully; saved delivery timestamps remained unchanged, confirming the retry tick ran without resending completed destinations. The earlier StartInterval/RunAtLoad deferral was also explained by launchd logs reporting the GUI domain in on-demand-only mode; explicit calendar events ran successfully.

...
function createConversationService({db,gmail,discord,now=()=>new Date(),fetchImpl=fetch,venueDirectory=null}){
 ...
 async function tryAutoLink(c,messages){
  ...
  try{const found=venueLinks.matchVenue(await venueDirectory.list(),messages);const v=found.venue;
   ...
   if(c.followUpDate&&c.followUpDate!==v.date)return {...c,venueLinkIssue:'Matching venue found, but its follow-up date differs. Link manually to review.'};
   const patch={venueId:v.id,venueName:v.name,venueCity:v.city,venueLinkMode:'automatic',venueLinkIssue:'',followUpDate:v.date,venueLinkedAt:now().toISOString()};

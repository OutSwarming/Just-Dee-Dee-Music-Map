...
function createConversationService({db,gmail,discord,now=()=>new Date(),fetchImpl=fetch,venueDirectory=null,excludeMessage=()=>false}){
 ...
 async function linkVenue(id,venueId,actor,expectedVenueId){
  ...
  await linkMemory.remember(updated.linkEvidence?.identities||[], 'email:'+id,venueId);await updateCard(updated);return updated;
 }
 ...
 async function poll(){const cfg=await config();if(!cfg.enabled)return {paused:true};const lock=db.doc('jddmEmailConfig/pollLock');const acquired=await db.runTransaction(async tx=>{const s=await tx.get(lock);if((s.data()?.until||0)>Date.now())return false;tx.set(lock,{until:Date.now()+500000});return true;});if(!acquired)return {busy:true};const result={conversations:0,posted:0};try{
  ...
  if(venueDirectory){try{result.linksRefreshed=await rematchSaved();await refreshLinked();}catch(e){result.venueRefreshError=e.message;}}
 ...
 return {config,get,syncThread,poll,setStatus:(...args)=>venueDirectory&&args[1]==='followup'?withVenueLock(args[0],()=>setStatus(...args)):setStatus(...args),setTopics,reply,daily,updateCard,tryAutoLink,linkVenue:(...args)=>withVenueLock(args[0],()=>linkVenue(...args)),refreshLinked,rematchSaved,searchVenues:q=>venueDirectory.search(q),browseVenues:async c=>{const index=await venueDirectory.list();const matches=venueLinks.matchVenue(index,[{payload:{headers:[{name:'From',value:c.correspondent||''},{name:'Subject',value:c.subject||''}]}}]).candidates;const priority=new Set([c.venueId,...(c.venueCandidates||[]),...matches.map(v=>v.id)].filter(Boolean));return index.filter(v=>v.linkable!==false).sort((a,b)=>Number(priority.has(b.id))-Number(priority.has(a.id))||a.name.localeCompare(b.name));}};
}
...
function createConversationInteractions({getConfig,service,discord,db,legacy,prefix='jddm2',readOnlySource=false}){return async(req,res)=>{
 ...
 if(action==='venue-search-submit'||action==='venue-search'){
  const browsing=action==='venue-search';
  if(!browsing&&value.length<2)throw Error('Enter at least two characters to search');
  const user=i.member?.user?.id;if(!user)throw Error('Your Discord account could not be verified');
  const results=browsing?await service.browseVenues(c):await service.searchVenues(value),offered=results.slice(0,24),sessionId=crypto.randomBytes(10).toString('hex');

...
const DIGEST_VERSION = '2026-09-12-current-three-day-window';
...
function buildDigest({rows=[],emails=[],worklist=[],booking=[],today=dateKey()}){
 ...
 const mail=emails.filter(e=>!e.venueId&&e.status==='followup').map(e=>({name:e.subject||e.correspondent||'Email conversation',date:calendarDate(e.followUpDate),url:`https://discord.com/channels/1543777084265070623/${e.discordThreadId}`})).filter(e=>e.date);
 // A current official date wins over every cached/suggested campaign date.
 const bookingItems=booking.map(b=>{const row=rows.find(r=>b.id&&r['Place ID']===b.id);return {...b,date:calendarDate(row?.['Next Follow Up'])||(row&&b.official?'':calendarDate(b.date))};}).filter(b=>b.date&&!b.attention&&!venues.some(v=>v.id===b.id)&&!rows.some(r=>b.id===r['Place ID']&&/^(Open Microphone|Told No)/i.test(r.Status||'')));
 ...
 const lines=['Hi Dee Dee, here are the places you need to contact today:',`Daily follow-ups — ${today} (Eastern)`,...due.map(v=>`• ${v.name}${links(v)}`)];
 ...
 lines.push('',APP);
 return {version:DIGEST_VERSION,today,body:lines.join('\n'),venues:due.length,bookingFollowUps:bookingDue.length,emails:dueMail.length,horizonPlaces:upcoming.length,horizonEmails:upcomingMail.length,followUpContext:{worklist,booking}};
}
async function loadDigest({db,fetchImpl=fetch,today=dateKey(),cache=true}){
 ...
 // Delivery receipts prevent duplicate sends. They must not freeze source dates:
 // each read checks the live sheet, including changes made after 8 AM.
 ...
 if(cache)return db.runTransaction(async tx=>{const s=(await tx.get(ref)).data();if(s?.version===DIGEST_VERSION&&s.body===digest.body&&JSON.stringify(s.followUpContext)===JSON.stringify(digest.followUpContext))return digest;if(!s?.checkedAt||s.checkedAt<=checkedAt)tx.set(ref,{...digest,createdAt:s?.createdAt||checkedAt,checkedAt});return digest;});
 return digest;
}
module.exports={buildDigest,loadDigest,dateKey,plusDays,DIGEST_VERSION};

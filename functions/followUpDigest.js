...
function buildDigest({rows=[],emails=[],today=dateKey()}){
 ...
 const venues=rows.map(v=>({name:String(v['Place Name']||'Unknown place'),date:calendarDate(v['Next Follow Up']),status:String(v.Status||'')})).filter(v=>v.date&&!/^(Booked|Played in the Past|Open Microphone|Told No)/i.test(v.status));
 const mail=emails.filter(e=>e.status==='followup'&&e.followUpDate).map(e=>({name:e.subject||e.correspondent||'Email conversation',date:e.followUpDate,url:`https://discord.com/channels/1543777084265070623/${e.discordThreadId}`}));
 ...
 const lines=['Hi Dee Dee, here are the places you need to contact today:',`Daily follow-ups — ${today} (Eastern)`,...due.map(v=>`• ${v.name}${v.date<today?' — overdue since '+v.date:''}`)];
 ...
 if(upcoming.length||upcomingMail.length)lines.push('','On the horizon — the next 2 days:',...upcoming.map(v=>`• ${v.name} — ${v.date}`),...upcomingMail.map(v=>`• Email: ${v.name} — ${v.date}\n${v.url}`));
 lines.push('',APP);
...
async function loadDigest({db,fetchImpl=fetch,today=dateKey(),cache=true}){
 ...
 const emails=(await db.collection('jddmEmailConversations').where('status','==','followup').get()).docs.map(d=>({...d.data(),status:'followup'}));

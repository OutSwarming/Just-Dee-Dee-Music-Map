...
function createService({db,discord,graph,venueDirectory,now=()=>new Date(),platform:platformName='messenger'}){
 ...
 const memory=identity.createMemory(db);
 ...
 async function tryAutoLink(c,messages=[]){
  if(!venueDirectory||c.testConversation||c.venueLinkMode==='manual')return c;
  try{const evidence=messages.length?identity.socialEvidence({...c,accountId:PAGE_ID},messages):c.linkEvidence||identity.socialEvidence({...c,accountId:PAGE_ID});
   const found=identity.resolve(await venueDirectory.list(),evidence,await memory.lookup(evidence.identities));const v=found.venue;
   const patch={linkEvidence:evidence,linkVersion:identity.VERSION,venueCandidates:found.candidates.slice(0,12).map(v=>v.id),venueLinkReason:found.reason,venueLinkIssue:v?'':found.reason,...(v?{venueId:v.id,venueName:v.name,venueCity:v.city,followUpDate:v.date,venueLinkMode:'automatic'}:c.venueLinkMode==='automatic'?{previousAutomaticVenueId:c.venueId||'',venueId:'',venueName:'',venueCity:'',followUpDate:'',venueLinkMode:'review'}:{})};
   return {...c,...patch};
  }catch{return {...c,venueLinkIssue:'Venue matching is temporarily unavailable; messages continue to arrive.'};}
 }
 async function updateCard(c){const cfg=await config(),render=card(c),renderHash=hash(JSON.stringify(render));const tags=[cfg.tags?.[c.status],...(c.importedHistory?[cfg.historyTag]:[]),...(c.topics||[]).map(t=>cfg.topicTags?.[t])].filter(Boolean).slice(0,5);const t=await discord('GET','/channels/'+c.discordThreadId);if(c.cardHash!==renderHash||JSON.stringify(tags)!==JSON.stringify(t.applied_tags)||t.name!==c.name.slice(0,100)){await discord('PATCH','/channels/'+c.discordThreadId,{archived:false,name:c.name.slice(0,100),applied_tags:tags});await discord('PATCH',`/channels/${c.discordThreadId}/messages/${c.starterId}`,render);await collection.doc(c.id).set({cardHash:renderHash},{merge:true});}if(['resolved','rejected','spam'].includes(c.status)&&(!t.thread_metadata?.archived||c.cardHash!==renderHash))await discord('PATCH','/channels/'+c.discordThreadId,{archived:true});}
 async function syncConversation(thread,{historical=false}={}){thread={...thread,sourceId:thread.id,id:platformName==='instagram'?hash(thread.id).slice(0,32):thread.id};return locked(thread.id,async()=>{
  ...
  if(await privacy.isRestricted(db,platformName,contacts[0].id))return {skipped:true,restricted:true};
  ...
  if(!c){c={id:thread.id,sourceId:thread.sourceId,platform:platformName,psid:contacts[0].id,username:contacts[0].username||'',name:contacts[0].name||contacts[0].username||platform.label+' contact',status:historical?'history':'deedee',topics:[],venueId:'',followUpDate:'',importedHistory:historical,createdAt:now().toISOString()};
  ...
  c={...c,username:contacts[0].username||'',name:contacts[0].name||contacts[0].username||c.name};
  let posted=0;
 ...
 async function refreshLinked(){const docs=await collection.where('venueId','>','').get();if(!docs.size)return;const index=await venueDirectory.list({fresh:true});const patchFor=c=>{const v=index.find(v=>v.id===c.venueId&&v.linkable!==false);return v?{venueName:v.name,venueCity:v.city,followUpDate:v.date,venueLinkIssue:''}:{venueLinkIssue:'Linked venue unavailable; choose an existing venue'};};for(const doc of docs.docs){const saved=doc.data();if(require('./operationStore').same(saved,{...saved,...patchFor(saved)}))continue;await locked(doc.id,async()=>{const c=await get(doc.id);if(!c)return;const patch=patchFor(c);if(!require('./operationStore').same(c,{...c,...patch})){await collection.doc(c.id).set(patch,{merge:true});await updateCard({...c,...patch});}});}}
 async function poll(){return locked('poll',async()=>{const cfg=await config();if(!cfg.enabled)return {paused:true};const profile=await graph('me?fields=id,name');if(profile.id!==PAGE_ID)throw Error('Wrong Facebook Page token');let processed=0,posted=0;if(platformName==='instagram'){const account=await graph(PAGE_ID+'?fields=instagram_business_account');if(account.instagram_business_account?.id!==platform.selfId)throw Error('Wrong linked Instagram account');}const threads=await allPages(graph,PAGE_ID+'/conversations?fields=id,updated_time,participants'+(platformName==='messenger'?',link':'')+'&limit=100'+(platformName==='instagram'?'&platform=instagram':''));const saved=db.__jddmOperationDb?new Map((await collection.get()).docs.map(d=>[d.id,d.data()])):null;for(const t of threads){const contact=t.participants?.data?.find(p=>p.id!==platform.selfId);if(contact&&await privacy.isRestricted(db,platformName,contact.id))continue;const id=platformName==='instagram'?hash(t.id).slice(0,32):t.id;let c=saved?saved.get(id):await get(id);if(c&&c.updatedTime&&c.updatedTime===t.updated_time&&(c.linkVersion===identity.VERSION||c.venueLinkMode==='manual'||c.testConversation)){const candidate={...await tryAutoLink(c),sourceLink:t.link||''};if(require('./operationStore').same(c,candidate)){await updateCard(c);continue;}await locked(c.id,async()=>{const current=await get(c.id);if(!current)return;const next={...await tryAutoLink(current),sourceLink:t.link||''};if(!require('./operationStore').same(current,next))await collection.doc(c.id).set(next);await updateCard(next);});continue;}t.messages=await allPages(graph,t.id+'/messages?fields=id,created_time,from,to,message,attachments,shares&limit=100');const result=await syncConversation(t,{historical:!cfg.importComplete});posted+=result.posted||0;processed++;}await refreshLinked();await db.doc(platform.namespace+'Config/main').set({lastSuccessAt:now().toISOString(),lastError:''},{merge:true});return {processed,posted,total:threads.length};});}
 return {get,config,tryAutoLink,updateCard,syncConversation,linkVenue,setStatus,setTopics,refreshLinked,poll,searchVenues:q=>venueDirectory.search(q),browseVenues:async c=>{const index=await venueDirectory.list(),priority=new Set([c.venueId,...(c.venueCandidates||[]),...require('./venueLinks').searchVenues(index,c.name||'').map(v=>v.id)]);return index.filter(v=>v.linkable!==false).sort((a,b)=>Number(priority.has(b.id))-Number(priority.has(a.id))||a.name.localeCompare(b.name));}};
}

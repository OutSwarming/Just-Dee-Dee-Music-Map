...
const quiet={parse:[]};
function sourceUrl(c){return c.sourceLink?.startsWith('/'+PAGE_ID+'/inbox/')?'https://www.facebook.com'+c.sourceLink:`https://business.facebook.com/latest/inbox/messenger?asset_id=${PAGE_ID}`;}
function card(c){
 ...
 rows[0].components.unshift({type:2,style:5,label:'Open Messenger / Reply',url:sourceUrl(c)});
 for(const row of rows)for(const x of row.components)if(x.custom_id)x.custom_id=x.custom_id.replace('jddm2:',PREFIX+':');
 return {content:state?`${state.emoji} **${state.name}**`:'📚 **Imported conversation — review when needed**',embeds:[{title:c.name.slice(0,256),color:state?.color||0x5865f2,description:`${c.venueId?'📍 '+c.venueName:'📍 No venue linked — use Link venue'}\n📅 Official spreadsheet follow-up: **${c.followUpDate||'not scheduled'}**${c.venueLinkIssue?'\n⚠️ '+c.venueLinkIssue:''}\n\nMessages and replies stay together in this post. Use **Open Messenger / Reply** to send through Facebook.`,footer:{text:'Just Dee Dee Music • Facebook Page Messenger • Eastern time'}}],components:rows,allowed_mentions:quiet};
}
function messageParts(m){
 const outgoing=m.from?.id===PAGE_ID;
 ...
 const body=`**${outgoing?'Sent by Just Dee Dee Music':'Received from '+(m.from?.name||'Messenger contact')}** · ${stamp} Eastern\n\n${m.message||'(No text in this message.)'}${attachments.length?'\n\n📎 '+attachments.join(', ')+' — open Messenger to view.':''}${shares.length?'\n\nShared: '+shares.join('\n'):''}`;
 return email.splitText(body,1850).map((content,i)=>({content,flags:4096,allowed_mentions:quiet,nonce:hash('messenger:'+m.id+':'+i).slice(0,24),enforce_nonce:true}));
}
...
function createService({db,discord,graph,venueDirectory,now=()=>new Date()}){
 const collection=db.collection('jddmMessengerConversations');
 const get=async id=>(await collection.doc(id).get()).data();
 const config=async()=>(await db.doc('jddmMessengerConfig/main').get()).data()||{};
 async function locked(id,fn){const ref=db.doc('jddmMessengerLocks/'+hash(id)),owner=crypto.randomUUID();await db.runTransaction(async tx=>{const d=(await tx.get(ref)).data();if(d?.until>Date.now())throw Error('Another Messenger update is running. Try again shortly');tx.set(ref,{owner,until:Date.now()+600000});});try{return await fn();}finally{await db.runTransaction(async tx=>{if((await tx.get(ref)).data()?.owner===owner)tx.set(ref,{until:0});});}}
 ...
 async function syncConversation(thread,{historical=false}={}){return locked(thread.id,async()=>{
  const cfg=await config();if(!cfg.forumId)throw Error('Messenger forum is not configured');
  const people=thread.participants?.data||[];if(!people.some(p=>p.id===PAGE_ID))throw Error('Conversation does not belong to JDDM');
  const contacts=people.filter(p=>p.id!==PAGE_ID);if(contacts.length!==1)throw Error('Expected one Messenger contact');
  ...
  if(!c){c={id:thread.id,psid:contacts[0].id,name:contacts[0].name||'Messenger contact',status:historical?'history':'deedee',topics:[],venueId:'',followUpDate:'',importedHistory:historical,createdAt:now().toISOString()};
  ...
  let posted=0;
  for(const m of messages){if(!m.id||!Number.isFinite(Date.parse(m.created_time)))throw Error('Invalid source message');const ref=db.doc('jddmMessengerMessages/'+hash(m.id));let receipt=(await ref.get()).data()||{};if(receipt.complete)continue;
   ...
   const parts=messageParts(m);for(let i=receipt.nextPart||0;i<parts.length;i++){const p=await discord('POST',`/channels/${c.discordThreadId}/messages`,parts[i]);receipt={...receipt,nextPart:i+1,messageIds:[...(receipt.messageIds||[]),p.id]};await ref.set({...receipt,conversationId:c.id,sourceMessageId:m.id});}
  ...
  if(newer){c={...c,lastMessageAt:Date.parse(latest.created_time),preview:(latest.message||'Attachment').slice(0,160),...(!historical?{status:latest.from?.id===PAGE_ID?'venue':'deedee'}:{})};}
...
module.exports={PAGE_ID,PREFIX,card,messageParts,createService,createGraphClient,allPages};

...
function card(r){const s=SOURCES[r.source],pending=r.status==='pending',state=r.status==='linked'?'🟢 Linked':r.status==='ignored'?'⚪ Ignored':r.status==='inactive'?'⚪ No longer active':'🟠 Needs a venue';
 ...
 return {content:`${s.emoji} **${s.label}${r.kind?' · '+r.kind:''}** — ${state}`,embeds:[{title:clean(r.name||'Unknown contact',180),color:r.status==='linked'?0x2ecc71:r.status==='ignored'?0x95a5a6:s.color,description:[r.correspondent?'Contact: '+esc(r.correspondent):'',r.location?'📍 '+esc(r.location):'',r.dates?.length?'Dates: '+r.dates.slice(0,8).join(', '):'',r.status==='linked'?'Linked to **'+esc(r.venueName)+'**. Official follow-up: **'+(r.followUpDate||'not scheduled')+'**.':r.status==='ignored'?'Ignored in Linking Review. The original messages stay untouched.':(r.reason||'Choose the existing spreadsheet venue, or review a new place in the app.').replace(/Use Link venue to search\./g,'Use Search venues below.'),r.searchQuery?'🔎 Results for **'+esc(r.searchQuery)+'** · '+choices.length+' shown.':'',r.feedback||'',pending?'Search opens a popup. Results and confirmations stay on this card. Add in app requires reviewing and saving the new place.':''].filter(Boolean).join('\n').slice(0,3800),footer:{text:'Linking Review'}}],components,allowed_mentions:{parse:[]},flags:4096};
}
...
function createService({db,discord,directory,services,calendar,now=()=>Date.now()}){
 ...
 async function publish(r){if(r.publishRetryAt>now()){await ref(r.id).set(r);return r;}const body=card(r),digest=hash(JSON.stringify(body));if(r.messageId&&r.messageHash===digest)return r;let m;
  try{m=await discord(r.messageId?'PATCH':'POST',r.messageId?`/channels/${CHANNEL}/messages/${r.messageId}`:`/channels/${CHANNEL}/messages`,{...body,...(!r.messageId?{nonce:r.id.slice(0,24),enforce_nonce:true}:{})});}catch(e){if(e.status===429){const queued={...r,publishRetryAt:now()+Math.ceil((e.retryAfter||60)*1000),publicationError:e.message};await ref(r.id).set(queued);return queued;}if(!r.messageId||e.status!==404)throw e;m=await discord('POST',`/channels/${CHANNEL}/messages`,{...body,nonce:r.id.slice(0,24),enforce_nonce:true});}
  const n={...r,messageId:m.id||r.messageId,messageHash:digest,publishRetryAt:0,publicationError:''};await ref(r.id).set(n);return n;}
 ...
 async function upsert(input,index){const id=key(input.source,input.sourceId);return locked(id,async()=>{const old=await get(id);if(old?.decisionAt&&old.decisionAt>=input.observedAt)return old;if(!old&&(input.testConversation||input.venueId&&!(input.source==='calendar'&&input.messageId)))return;
  let r={...old,...input,...(old?.messageId?{messageId:old.messageId}:{}),id,draftToken:old?.draftToken||crypto.randomBytes(16).toString('hex'),draftPlaceId:'linking-review-'+id,revision:old?.revision||0,selectionVersion:old?.selectionVersion||0,status:input.venueId?'linked':(input.source==='calendar'?input.status==='ignored':old?.status==='ignored')?'ignored':input.active===false?'inactive':'pending'};

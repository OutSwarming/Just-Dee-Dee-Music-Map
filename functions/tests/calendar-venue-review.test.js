...
test('New row is explicit and duplicate confirmation is rejected; retries after ambiguous creation recover by stable ID',async()=>{const s=setup(),e=event(),id=keyFor(e);await s.service.resolve([e]);await s.service.choose(id,'new','','user',0);assert.equal(s.writes.length,1);assert.equal((await s.service.resolve([e])).mappings[id],'calendar-approved-'+id);await assert.rejects(s.service.choose(id,'new','','user',0),/already changed/);assert.equal(s.writes.length,1);
const t=setup();await t.service.resolve([e]);t.rows.push(row('calendar-approved-'+id,e.venueName));await t.service.choose(id,'new','','user',0);assert.equal(t.writes.length,0);
});
test('a venue added meanwhile blocks New row; duplicate-ID targets cannot be linked',async()=>{const s=setup(),e=event(),id=keyFor(e);await s.service.resolve([e]);s.rows.push(row('a',e.venueName));await assert.rejects(s.service.choose(id,'new','','user',0),/existing venue/);s.rows.push(row('a','Other'));await assert.rejects(s.service.choose(id,'link','a','user',0),/duplicated/);assert.equal(s.writes.length,0);});
...
test('two concurrent confirmations can only create one row',async()=>{const s=setup(),id=keyFor(event());await s.service.resolve([event()]);const out=await Promise.allSettled([s.service.choose(id,'new','','a',0),s.service.choose(id,'new','','b',0)]);assert.equal(out.filter(r=>r.status==='fulfilled').length,1);assert.equal(s.writes.length,1);});
